...
The Cross-Storage Move feature enables TFM (TUI File Manager) to move files and directories between different storage systems seamlessly. This includes moving files between local filesystem and S3, between different S3 buckets, and other remote storage systems.
...
- Determines optimal move strategy based on storage compatibility
- Provides user feedback about cross-storage operations

### 2. Intelligent Move Operations
- **Same-storage moves**: Uses native rename operations for efficiency
- **Cross-storage moves**: Uses copy-then-delete strategy for reliability
- **Progress tracking**: Shows progress for large operations
- **Error handling**: Comprehensive error handling and recovery

### 3. User Experience
- Seamless integration with existing TFM move operations
- Visual indicators for cross-storage operations
- Confirmation dialogs with storage type information
...
### User Interface
- Press `M` or `m` to move selected files
- Cross-storage moves show additional information:
  ```
...
### Progress Display
For large operations, TFM shows:
- Current file being processed
- Progress percentage
- Transfer speed (for cross-storage)
...
### Architecture
- `Path.move_to()` method handles all move operations
- Storage-specific implementations in `PathImpl` subclasses
- Cross-storage logic in base `Path` class
