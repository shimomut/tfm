...
| `truncate_to_width(s, w[, ellipsis])` | `puikit.text.truncate_to_width(s, w, ellipsis[, measure])` | **DONE (upstreamed)** | PuiKit's `truncate_to_width` gained `ellipsis` **and an optional `measure`**. **Also added `puikit.text.elide(s, w, ellipsis, where, measure)`** (end/middle/start, glyph-aware), covering TFM's `AbbreviationSegment` (middle/left/right). Crucially both take a `measure` callback (default `display_width`): pass `ctx.measure_text` to fit a **proportional GUI font by real width**, not column count — TTK was monospace-only. So when the FilePane/viewer widgets render proportional text they fit it correctly; `tfm_text_layout`'s grid abbreviation folds onto `elide` then. Demoed on the catalog's **Truncate** page. |
