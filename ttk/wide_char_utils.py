#!/usr/bin/env python3
"""
Wide Character Utilities for TTK

This module provides utilities for handling wide characters (particularly East Asian
characters) in terminal and desktop display contexts. It includes functions for 
calculating display width, text truncation, padding, and capability detection.

The module addresses layout issues that occur when text contains wide Unicode characters
by providing proper display width calculations and text manipulation functions.

This module is backend-agnostic and works with both terminal (curses) and desktop
...
def initialize_wide_char_utils(
    ...
    try:
        ...
        # Set fallback character
        set_fallback_char(fallback_char)
        
    except Exception as e:
        warnings.warn(f"Error loading Unicode settings from config: {e}. Keeping current mode.", UserWarning)


# Global fallback character configuration
_fallback_char = '?'


def set_fallback_char(char: str):
    """
    Set the fallback character for unrepresentable characters.
    
    Args:
        char: Single character string to use as fallback
    """
    global _fallback_char
    if isinstance(char, str) and len(char) == 1:
        _fallback_char = char


def get_fallback_char() -> str:
    """
    Get the fallback character for unrepresentable characters.
