...
- [x] 1. Add data structures for progressive scanning
  - [x] 1.1 Add scanning state fields to TreeNode
  ...
  - [x] 1.4 Create ComparisonTask dataclass
    ├── Define fields: left_path, right_path, relative_path, priority, is_visible
    ├── Used for queuing file comparison operations
    └── _Requirements: US2, US4_

- [x] 2. Add thread synchronization primitives
  - [x] 2.1 Add locks for thread safety
  ...
  - [x] 2.3 Add worker thread management
...
- [x] 3. Implement single-level directory scanning
  - [x] 3.1 Create _scan_single_level() method
  ...
  - [x] 3.2 Modify start_scan() for top-level only
...
- [x] 4. Checkpoint - Verify immediate tree display
  └── Ensure tree displays within 100ms with top-level items visible

- [x] 5. Implement directory scanner worker thread
  - [x] 5.1 Create _directory_scanner_worker() method
  ...
  - [x] 5.2 Create _start_directory_scanner_worker() method
    ├── Create and start scanner thread
    ├── Store thread reference
    └── _Requirements: US2_

  - [x] 5.3 Implement thread-safe tree updates
...
- [x] 6. Implement file comparator worker thread
  - [x] 6.1 Create _file_comparator_worker() method
  ...
  - [x] 6.2 Create _start_file_comparator_worker() method
    ├── Create and start comparator thread
    ├── Store thread reference
    └── _Requirements: US2_

  - [x] 6.3 Modify build_tree() to queue file comparisons
...
- [x] 7. Checkpoint - Verify background scanning works
  └── Ensure tree updates progressively as directories are scanned

- [x] 8. Implement priority system ✓ COMPLETED
  - [x] 8.1 Define priority levels ✓
...
- [x] 9. Implement on-demand scanning for user expansion
  ...
  - [x] 9.2 Add loading indicator during on-demand scan
    ├── Show "scanning..." text while scan_in_progress is True
    ├── Update status bar with current operation
    └── _Requirements: US3, US5_

- [x] 10. Implement lazy scanning for one-sided directories
  - [x] 10.1 Modify _directory_scanner_worker() to skip one-sided dirs
  ...
  - [x] 10.2 Ensure expand_node() handles one-sided directories
    ├── When user expands one-sided directory, scan it immediately
    ├── Update tree with contents
    └── _Requirements: US3_

- [x] 11. Checkpoint - Verify on-demand and lazy scanning
  └── Ensure expanding unscanned directories works correctly

- [x] 12. Add visual indicators for pending status
  - [x] 12.1 Update _get_node_display_text() for pending indicators
  ...
  - [x] 12.2 Update status bar for scanning progress
...
- [x] 18. Update documentation
  - [x] 18.1 Update DIRECTORY_DIFF_VIEWER_FEATURE.md
    ├── Document progressive scanning behavior
    ├── Explain pending status indicators
    ├── Note performance improvements
    └── _Requirements: US1, US2, US5_

  - [x] 18.2 Update DIRECTORY_DIFF_VIEWER_IMPLEMENTATION.md
...
- [x] 19. Final checkpoint - Integration and performance testing
